...
# ILLUMINATE.SH handles the downloading, configuration, and building of everything necessary
...
chk_ddctl() {
  if [ -d "$esrc/ddcutil-2.2.4" ] || [ -d "$esrc/ddcutil-2.2.5" ]; then
    printf "\n$bold%s $off%s\n" "Updating ddcutil..."

    if [ -d "$esrc/ddcutil-2.2.4" ]; then
      olddir="$esrc/ddcutil-2.2.4"
    elif [ -d "$esrc/ddcutil-2.2.5" ]; then
